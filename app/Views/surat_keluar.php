...
<div class="main">
  ...
  <div class="content">
    <a href="/surat/tambahKeluar" class="btn btn-green">+ Tambah Surat Keluar</a>

    <!-- 🔎 Form Pencarian & Filter -->
    <form method="get" action="/surat/keluar" class="filter-form">
    ...
    <table>
      ...
      <tbody>
        <?php if (!empty($suratKeluar)) : ?>
          <?php $no=1; foreach ($suratKeluar as $row): ?>
          <tr>
            <td><?= $no++ ?></td>
            ...
            <td>
              <?php if ($row['file_surat']) : ?>
                <a href="/uploads/keluar/<?= esc($row['file_surat']) ?>" target="_blank" class="btn btn-blue">Lihat</a>
              <?php endif; ?>
            </td>
            <td>
              <a href="/surat/editKeluar/<?= $row['id'] ?>" class="btn btn-yellow">Edit</a>
              <a href="/surat/hapusKeluar/<?= $row['id'] ?>" class="btn btn-red" onclick="return confirm('Hapus data ini?')">Hapus</a>
            </td>
          </tr>
          <?php endforeach ?>
        <?php else : ?>
