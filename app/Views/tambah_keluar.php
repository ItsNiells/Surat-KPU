...
<div class="main">
  <div class="header">📤 Tambah Surat Keluar</div>
  <div class="card">
    <form action="/surat/simpanKeluar" method="post" enctype="multipart/form-data">
      ...
      <label>Tujuan</label>
      <input id="kolomtambah" type="text" name="tujuan" required>

      <label>Tanggal Keluar</label>
      <input id="kolomtambah" type="date" name="tanggal_keluar" required>
...
<script>
document.getElementById("fileKeluar").addEventListener("change", function() {
  const fileName = this.files[0] ? this.files[0].name : "Belum ada file dipilih";
  document.getElementById("fileNameKeluar").textContent = fileName;
});
</script>
